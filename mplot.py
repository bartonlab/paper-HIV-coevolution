...
def plot(**pdata):
    ...
    if 'x' in pdata and 'y' in pdata:
        ndata = len(pdata['x'])
        assert ndata==len(pdata['y']), 'x (dimension %d) and y (dimension %d) have mismatched shapes!' % (ndata, len(pdata['y']))
...
def error(**pdata):
    ...
    for i in range(len(pdata['x'])):
        ...
        if 'xerr' in pdata and np.any(pdata['xerr'][i]): xerr = pdata['xerr'][i]
        if 'yerr' in pdata and np.any(pdata['yerr'][i]): yerr = pdata['yerr'][i]
