...
#
function get_d_rec_correction(q, L, qL, qqLLhalf, Dx_k, indices_mapped)
    d_rec_corr = zeros(qL+qqLLhalf) # elements in 1:qL should be zeros
    DxDxt = Dx_k * Dx_k'
    for i in 1:L
        for a in 1:q
            u = km(i,a,q)
            for j in (i+1):L
                for b in 1:q
                    ...
                    #this should be modified for categorical variables case. 
                    i_eff, j_eff = indices_mapped[i], indices_mapped[j]
		    d_rec_corr[ξ] = (j_eff - i_eff)*DxDxt[u,v]
                end
            end
        end
    end
